...
class utilisateur extends connect
{
	...
	function habilitationButton($url, $men_getText, $prf_code)
	{
		...
		$sql = "SELECT * 
				FROM vw_profil_menu 
				WHERE mem_parent2 = (SELECT men_id FROM vw_profil_menu WHERE men_url = '$url' AND prf_code = $prf_code)
	...
	function habilitationMenu($url,  $prf_code)
	{
		...
		$sql = "SELECT * 
				FROM vw_profil_menu 
				WHERE men_id = (SELECT men_id FROM vw_profil_menu WHERE men_url = '$url' AND prf_code = $prf_code)
				";
